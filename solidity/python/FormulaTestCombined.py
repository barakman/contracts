...
def formulaTest(supply,reserve,ratio,amount):
    newamount = Decimal(calculatePurchaseReturn(supply,reserve,ratio,amount))
    oldamount = Decimal(calculateSaleReturn(supply+newamount,reserve+amount,ratio,newamount))
    if oldamount > amount:
        error = []
